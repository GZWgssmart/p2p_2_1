...
<script type="text/javascript">
    ...
    layui.use(['form'], function () {
        ...
        $.get('<%=path %>/data/Role/roleDetail',{roleId:url},function (data) {
            $('#roleName').text(data.rname);
            $('#roleContent').text(data.content);
        });
        ...
        var xtree2 = new layuiXtree({
            ...
            , data: '<%=path %>/rolejur/all?rid='+url //(必填) 数据接口，需要返回以上结构的json字符串
            , click: function (data) {  //节点选中状态改变事件监听，全选框有自己的监听事件
                if(data.elem.checked){
                    $.post('<%=path %>/rolejur/addRoleJur',
                        {rid:url,jid:data.value},
                        function (res) {}
                    );
                } else {
                    $.post('<%=path %>/rolejur/remove',
                        {rid:url,jid:data.value},
                        function (res) {}
                    );
                }
